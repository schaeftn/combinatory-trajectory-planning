...
trait RunDecomposition extends LazyLogging with AkkaImplicits with JtsUtils {
 ...
 //val machinedGeo = wktReader.read("""POLYGON ((10 0, 30 0, 30 -15, 10 -15, 10 0))""")
  val scene = Cnc2DModel(boundaries = List(0.0f, 50.0f, -15.0f, 50.0f),
    ...
    lines.next() match {
      ...
      case inputString if inputString.equals("it") =>
        def getResults(accList: List[(Int, PathCoverageResult)], i: Int): List[(Int, PathCoverageResult)] = {
          val tree = s"${l.last.terms.index(i)}".startsWith("Tree(GenericCompositionPcStep,org.combinators.ctp.repositories.toplevel.PathCoverageStep & pFct,ArraySeq(Tree(ZigZagStep")
          if (accList.size > 1) {
            accList
          } else {
            if (tree) {
              ...
              val newList = if (percentage < 0.05 && tree) {
                accList :+ (i, pcr)
              }
              else {
                accList
