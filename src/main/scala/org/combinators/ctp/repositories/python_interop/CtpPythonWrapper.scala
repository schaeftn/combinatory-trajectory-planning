...
case class SubstitutionSchema(f: Map[String, String], substitutes: Map[String, String])
  ...
  def merge(tScheme: SubstitutionSchema): SubstitutionSchema =
    SubstitutionSchema(self.f ++ tScheme.f, self.substitutes ++ tScheme.substitutes) //overwrites if key exists
}

object SubstitutionSchema {
  def apply(fileList: Map[String, String], //File in, File out
            substitutes: Map[String, String]) //Subst in, Subst out
  = new SubstitutionSchema(fileList, substitutes) {
  }
}

case class ModifierPlannerScheme[A, B](st: SubstitutionSchema, pf: (A, String) => B, startFile: String)
case class PlannerScheme[B](st: SubstitutionSchema, pf: (String) => B, startFile: String)

abstract class PythonWrapper[B](t: SubstitutionSchema, startFile: String) extends LazyLogging {
  def generateFiles(): Unit = {
    t.executeTemplating()
  }

  def executePythonFile(args: String): String = {
    val pythonProcess = s"python3 $startFile $args"
    pythonProcess.lineStream_!.takeWhile(_ => true).toList.mkString("\r\n")
  }
}

abstract case class SimplePythonWrapper[B](t: SubstitutionSchema, startFile: String, args: String ="")
...
abstract case class PythonWrapperModifier[A, B](t: SubstitutionSchema, startFile: String)
  extends PythonWrapper[B](t, startFile) {
...
object PythonWrapper {
  def apply[A, B](t: SubstitutionSchema,
                  startFile: String,
