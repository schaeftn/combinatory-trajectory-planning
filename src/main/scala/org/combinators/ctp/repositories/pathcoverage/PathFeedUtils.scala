...
trait PathFeedUtils extends JtsUtils with PathRefinement {
  ...
  lazy val maxVfByToolAndAngle = (withMaxVfByAngle.zip(toolList)).map {
  ...
  lazy val withMaxVfByAcc: List[List[List[Float]]] = {
    ...
    def buildAccSegments(currentCoord: List[Float],
      ...
      val projX = projXFactor * newAccSeqX.lastOption.map{_._3}.getOrElse(0.0f)
      val projY = projYFactor * newAccSeqY.lastOption.map{_._3}.getOrElse(0.0f)
    ...
    def resolveAccLists(p1: List[Float], p2: List[Float],
      ...
      if (distanceBetweenPoints >
        ...
        val decPoints = decUpdatedS.dropWhile {
          case (v, t, s) =>
            s < accPoints.lastOption.map{_._3}.getOrElse(0.0f) }
    ...
    val updatedPathList = forwardAccs.map(i => i.map {
      _._1
    }).zip(toolList)

    val updatedReversedPathList = updatedPathList.map(singlePath => (singlePath._1.reverse, singlePath._2))
    val withDecelleration: List[List[(List[Float], List[(Float, Float, Float)])]] =
      getAccListsForStrategy(updatedReversedPathList).map {
      _.reverse
    }.map(_.tail) //for every path: skip first element which does not contain a deceleration tuple.
    // This way, pairs can be formed via zip

      // Liste von KnotenPaaren mit jeweils 2 Accfunctions
     val coordPairsWithAccelDecel = (forwardAccs.map { nodesWithAcc => nodesWithAcc.map{_._1} } zip
        withDecelleration.map(nodesWithDecel => nodesWithDecel.map{_._1})) zip
    ...
    coordPairsWithAccelDecel.map {
      ...
      }.reduceLeftOption(_ ++ _.tail).getOrElse{List.empty[List[Float]]}
    }
  }
