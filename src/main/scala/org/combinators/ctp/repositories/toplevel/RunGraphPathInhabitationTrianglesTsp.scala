...
import org.combinators.ctp.repositories.{cmp_scene_graph_path, cmp_scene_triangulation_parameters, dimensionality_two_d_t, sd_polygon_scene_type, sd_seg_triangles_para, _}
...
object RunGraphPathInhabitationTrianglesTsp extends App with LazyLogging with AkkaImplicits {
  ...
  val ihBatch = Gamma.InhabitationBatchJob[Properties](p_unityConnectionProperties_type)
    ...
    .addJob[(Graph[List[Float], WUnDiEdge], MpTaskStartGoal) =>
      (Seq[List[Float]], Seq[WUnDiEdge[List[Float]]], Float)](Constructor("graphTsp"))
    .addJob[(Scene, MpTaskStartGoal) => TriangleSegPath](triangulation_path_prog_type :&: sd_seg_triangles_para :&: Constructor("graphTsp"))
